...
import { ArrowUpLeft, Building2, ClipboardList, KeyRound, Users } from 'lucide-react';
...
export default async function DashboardPage() {
  ...
  const activity = [...(latestSales ?? []).map((item) => ({ id: `sale-${item.id}`, label: item.title, kind: 'عرض بيع', status: item.status })), ...(latestRequests ?? []).map((item) => ({ id: `request-${item.id}`, label: item.customer_name, kind: 'طلب عميل', status: item.status ?? 'new' }))].slice(0, 5);
